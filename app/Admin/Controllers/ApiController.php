...
class ApiController extends AdminController
{
    ...
    public function productionPlan(Request $request)
    {
        ...
        foreach ($list as $key => $value) {
            ...
            $obj->mql = count($value->orders) ? implode(',', $value->orders->pluck('mql')->toArray()) : $value->mql;
            $value->qr_code = json_encode($obj);
            $data[$key] = array_merge($value->toArray(), $order ? $order->toArray() : []);
            $data[$key]['mql'] = count($value->orders) ? implode(',', $value->orders->pluck('mql')->toArray()) : $value->mql;
