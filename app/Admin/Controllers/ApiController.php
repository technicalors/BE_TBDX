...
class ApiController extends AdminController
{
    ...
    public function warehouseFGLog(Request $request)
    {
        ...
        foreach ($records as $key => $record) {
            ...
            $record->so_ngay_ton = $record->sl_ton ? $this->datediff($record->created_at, Carbon::now()->format('Y-m-d H:i:s')) : "";
        }
        return $this->success(['data' => $records, 'totalPage' => $totalPage]);
    }
    ...
    public function exportWarehouseFGLog(Request $request)
    {
        ...
        foreach ($records as $key => $record) {
            ...
            $obj->so_ngay_ton = $obj->sl_ton ? $this->datediff($record->created_at, Carbon::now()->format('Y-m-d H:i:s')) : "";
