...
class ApiController extends AdminController
{
    ...
    public function manualList(Request $request)
    {
        ...
        foreach ($info_lo_sx as $info) {
            ...
            $obj->layout_id = $obj->order->layout_id ?? '';
            $data[] = $obj;
        }
        $order = [1, 0, 2, 3, 4];
        usort($data, function ($a, $b) use ($order) {
        ...
        if (count($fg_exports) > 0) {
            foreach ($fg_exports as $key => $fg_export) {
                $lsx_pallets = LSXPallet::with(['warehouseFGLog' => function ($logQuery) {
                    ...
                    ->get();
                $so_luong_da_xuat = LSXPallet::where('order_id', $fg_export->order_id)->where('status', LSXPallet::EXPORTED)->select('*', DB::raw('so_luong - remain_quantity as da_xuat'))->get()->sum('da_xuat');
                // return $so_luong_da_xuat;
    ...
    public function deleteTems(Request $request)
    {
        $input = $request->all();
        $tems = Tem::whereIn('id', $input ?? [])->get();
        $all = count($tems);
        $count = 0;
        ...
        return $this->success('', 'Đã xoá ' . $count . '/' . $all);
    }
